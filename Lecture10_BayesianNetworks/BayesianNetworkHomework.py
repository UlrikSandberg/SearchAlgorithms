...
class BayesianNetwork(object):
    ...
    def get_joint_probability(self, values):
        ...
        print values
        joint = 1
        a = reversed(self.get_variables())
        for i in a:
            joint = joint * i.get_probability(values[i.name], self.sub_vals(i, values))
...
def sprinkler():
    ...
    damageTire = Variable('DamagedTire', ('false', 'true'), dt)
    fuelTankLeaking = Variable('FuelTankLeaking', ('false', 'true'), ftl)
    electronicsMalfunctioning = Variable('ElectronicsMalfunctioning', ('false', 'true'), em)
    vibrations = Variable('Vibrations', ('false', 'true'), v, [damageTire])
    slowMaxSpeed = Variable('SlowMaxSpeed', ('false', 'true'), sms, [damageTire, electronicsMalfunctioning])
    highConsumption = Variable('HighConsumption', ('false', 'true'), hc, [damageTire, electronicsMalfunctioning, fuelTankLeaking])


    variables = [damageTire, fuelTankLeaking, electronicsMalfunctioning, vibrations, slowMaxSpeed, highConsumption]
    ...
    conditionals_evidents = {'HighConsumption': 'true'}

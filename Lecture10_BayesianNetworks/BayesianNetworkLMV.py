    ...
    def calculate_marginal_probability(self):
        ...
        if len(self.parents) == 0:
            ...
            for key, v in self.probability_table.items():
                ...
                value = [self.marginal_probabilities[j] + v[j] * parents_probability
                         for j in range(len(self.assignments))]

                print(parents_probability)
                self.marginal_probabilities = [
                    self.marginal_probabilities[j] + v[j]*parents_probability
                    for j in range(len(self.assignments))
                ]
                print(self.marginal_probabilities)
...
def sprinkler():
    ...
    vTable = {
        ...
        ('true', 'false'): (0.5, 0.5),
        ('false', 'true'): (0.7, 0.3),
        ('false', 'false'): (0.95, 0.05)
    }

    l = Variable('l', ('false', 'true'), lTable)
    m = Variable('m', ('false', 'true'), mTable)
    v = Variable('v', ('false', 'true'), vTable, [l, m])
    variables = [l, m , v]
